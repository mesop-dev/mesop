          ...
          for command in runtime().context().commands():
            if command.HasField("navigate"):
              ...
              if command.navigate.open_in_new_tab:
                break
              # Use the navigate command's explicit query_params if provided,
              # otherwise extract them from the URL string. This handles
              # me.navigate("/path?X=1") where params are in the URL but not
              # passed via the query_params argument.
              nav_query_params = command.navigate.query_params
              if not nav_query_params:
